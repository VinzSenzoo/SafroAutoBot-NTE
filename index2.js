...
async function loadAccounts() {
  try {
    ...
    accounts = await Promise.all(mnemonics.map(async (mnemonic, index) => {
      if (mnemonic.split(' ').length < 12) {
        addLog(`Invalid mnemonic for account #${index + 1}: Too short (must be at least 12 words)`, "error");
        return null;
      }
      const wallet = await DirectSecp256k1HdWallet.fromMnemonic(mnemonic, { prefix: BECH32_PREFIX });
      const [acc] = await wallet.getAccounts();
      return { mnemonic, address: acc.address, token: null };
    }));
    accounts = accounts.filter(account => account !== null); // Filter out invalid ones
    if (accounts.length === 0) {
      throw new Error("No valid mnemonics found in seed.txt");
    }
    addLog(`Loaded ${accounts.length} accounts from seed.txt`, "success");
  } catch (error) {
...
async function performSend(mnemonic, fromAddress, toAddress, amount, proxyUrl) {
  if (shouldStop) throw new Error("stopped");
  activeProcesses++;
  let client = null;
  try {
  ...
  try {
    for (let accountIndex = 0; accountIndex < accounts.length && !shouldStop; accountIndex++) {
      ...
      for (let sendCount = 0; sendCount < dailyActivityConfig.sendRepetitions && !shouldStop; sendCount++) {
        ...
        try {
          const result = await performSend(mnemonic, address, toAddress, parseFloat(amount), proxyUrl);
